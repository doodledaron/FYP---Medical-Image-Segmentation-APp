...
export const QuizSummary: React.FC<QuizSummaryProps> = ({
  ...
  // Track mount/unmount
  useEffect(() => {
    console.log("QuizSummary MOUNTED", { title, score, totalPoints });
    return () => {
      console.log("QuizSummary UNMOUNTED", { title });
    };
  }, [title, score, totalPoints]);
            ...
            return (
              <div key={question.id} className={`border-2 rounded-xl p-6 ${bgColor} transition-all hover:shadow-md`}>
                ...
                <div className="space-y-3 pl-10">
                  <div className={`p-3 rounded-lg ${correctness === 'correct' ? 'bg-green-100' :
                      correctness === 'incorrect' ? 'bg-white' : 'bg-white'
                    }`}>
                    <p className={`font-medium ${correctness === 'correct' ? 'text-green-700' :
